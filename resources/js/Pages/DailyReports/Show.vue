<template>
  <AppLayout>
    <div class="max-w-3xl">
      ...
      <Link
        :href="backUrl"
        class="text-sm text-gray-500 hover:text-gray-800 flex items-center gap-1 mb-4"
      >
      ...
      <div class="bg-white rounded-lg border border-gray-200 p-6 mb-6">
        ...
        <!-- 学習内容 -->
        <div class="mb-5">
          <h2 class="text-xs font-semibold text-gray-400 uppercase tracking-wider mb-2">学習内容</h2>
          <p class="text-sm text-gray-800 whitespace-pre-wrap leading-relaxed">{{ report.content }}</p>
        </div>

        <!-- 感想・気づき -->
        <div v-if="report.impression">
          <h2 class="text-xs font-semibold text-gray-400 uppercase tracking-wider mb-2">感想・気づき</h2>
          <p class="text-sm text-gray-800 whitespace-pre-wrap leading-relaxed">{{ report.impression }}</p>
        </div>
        <div v-else>
          <h2 class="text-xs font-semibold text-gray-400 uppercase tracking-wider mb-2">感想・気づき</h2>
          <p class="text-sm text-gray-400">（記入なし）</p>
        </div>
      </div>
      ...
      <div class="bg-white rounded-lg border border-gray-200 p-6">
        <h2 class="text-sm font-semibold text-gray-700 mb-4">
          コメント
          <span class="text-xs font-normal text-gray-400 ml-1">{{ report.comments?.length ?? 0 }} 件</span>
        </h2>
        ...
        <div v-if="canComment" class="border-t border-gray-100 pt-5">
