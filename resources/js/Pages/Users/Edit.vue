<template>
  <AppLayout>
    <div class="max-w-2xl">
      <h1 class="text-2xl font-bold text-slate-900 tracking-tight mb-6">ユーザーを編集</h1>
      ...
      <form @submit.prevent="submit" class="space-y-5 bg-white rounded-xl shadow-sm ring-1 ring-slate-900/5 p-6">
        ...
        <div>
          <label class="block text-sm font-medium text-slate-700 mb-1">パスワード（変更する場合のみ）</label>
          <input
...
function submit(): void {
  form.put(`/users/${props.targetUser.id}`);
}
</script>
