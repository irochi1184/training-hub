<template>
  <AppLayout>
    <div class="max-w-3xl">
      <!-- 戻るリンク -->
      <Link href="/tests" class="text-sm text-gray-500 hover:text-gray-800 flex items-center gap-1 mb-4">
      ...
      <div class="bg-white rounded-lg border border-gray-200 p-6 mb-6">
        <h1 class="text-lg font-bold text-gray-900 mb-1">{{ submission.test?.title }}</h1>
        ...
        <div class="flex items-center gap-6">
          <div class="text-center">
            ...
            <p class="text-sm text-gray-500 mt-1">得点 / {{ totalScore }} 点満点</p>
          </div>
          <div class="flex-1">
            <!-- スコアバー -->
            ...
            <p class="text-sm text-gray-500 mt-2">正解率: {{ scorePercent }}%</p>
          </div>
        </div>
      </div>
      ...
      <!-- 問題別正誤 -->
      <div class="space-y-4">
        <h2 class="text-sm font-semibold text-gray-700">問題別の結果</h2>
        ...
        >
          ...
          >
            <div class="flex items-center gap-2">
              <!-- 正誤アイコン -->
              <svg v-if="answer.is_correct === true" class="w-4 h-4 text-green-600" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.5">
          ...
          <!-- 問題内容 -->
          <div class="px-5 py-4">
            <p class="text-sm text-gray-800 font-medium mb-3">{{ answer.question?.body }}</p>
            ...
            <div class="space-y-2">
              ...
              >
                ...
                <!-- 正解マーク -->
                <span v-if="choice.is_correct" class="ml-auto text-xs text-green-600 font-medium">正解</span>
              </div>
            </div>
          </div>
        </div>
...
function answerHeaderClass(isCorrect: boolean | null): string {
  if (isCorrect === true) return 'bg-green-50 text-green-800';
...
function choiceClass(choice: Choice, answer: Answer): string {
  ...
  if (isCorrect && isSelected) return 'bg-green-50 text-green-800';
  if (isCorrect && !isSelected) return 'bg-green-50 text-green-700 opacity-70';
